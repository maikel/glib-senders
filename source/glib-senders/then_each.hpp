...
namespace gsenders {
  namespace then_each_ {
    ...
    struct next_item_t {
      template <class Item>
      using __f = decltype(then(__declval<Item>(), __declval<apply_fun<Fun>>()));
    };
    ...
    struct receiver {
      ...
      struct __t {
        ...
        friend auto tag_invoke(set_next_t, __t& self, Item&& item) noexcept {
          return set_next(
            self.op_->rcvr_, then(static_cast<Item&&>(item), apply_fun<Fun>{&self.op_->fun_}));
        }
    ...
    struct then_each_t {
      ...
      template <class Fun>
      constexpr auto operator()(Fun fun) const noexcept -> __binder_back<then_each_t, Fun> {
        return {{}, {}, {static_cast<Fun&&>(fun)}};
      }
    };
  } // namespace then_each_

  inline constexpr then_each_::then_each_t then_each{};
}