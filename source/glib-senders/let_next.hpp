...
namespace gsenders {
  namespace let_value_each_ {
    ...
    using let_value_t = next_signatures<set_next_t(decltype(stdexec::let_value(
      stdexec::__declval<Item>(),
      stdexec::__declval<apply_fun<Fun>>())))>;
    ...
    struct sender {
      ...
      template <class Env>
      friend auto
        tag_invoke(stdexec::get_completion_signatures_t, const sender& self, const Env& env)
